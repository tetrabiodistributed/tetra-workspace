FROM kasmweb/core-alpine-320:1.16.1
USER root
...
RUN apk update && apk -y install software-properties-common && add-apk-repository --yes --update ppa:ansible/ansible && apk install -y ansible && rm -rf /var/lib/apk/lists/*
